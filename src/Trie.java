...
public class Trie {
    ...
    public static void main(String[] args) throws IOException {
        Trie tree = new Trie();
        Scanner cin = new Scanner(new FileInputStream(new File("data3.in")));
        OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(new File("data3.out")));
        int n = cin.nextInt();
        for (int i = 1; i <= n; i++) {
            tree.insert(cin.next());
        }
        int m = cin.nextInt();
        for (int i = 1; i <= m; i++) {
            writer.write(tree.has(cin.next()) ? "YES" : "NO");
            writer.write('\n');
        }
        writer.close();
